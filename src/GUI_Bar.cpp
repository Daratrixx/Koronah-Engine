...
void GUI_Bar::loadBar() {
    for (unsigned int i = 0; i < GUI_MODE_COUNT; i++) {
        ...
        float data[8];
        data[0] = ((m_position[i].x + m_barMargin[i].x) / getGraphicSetting()->screenWidth) * 2 - 1;
        data[1] = ((m_position[i].y + m_barMargin[i].y) / getGraphicSetting()->screenHeight) * 2 - 1;
        data[2] = (((m_size[i].x - m_barMargin[i].x * 2) * getBarPercent()) / getGraphicSetting()->screenWidth) * 2;
        data[3] = ((m_size[i].y - m_barMargin[i].y * 2) / getGraphicSetting()->screenHeight) * 2;
